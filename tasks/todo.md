...
- [ ] **Paso 6: validación E2E** sobre 5 empresas T3 reales (NO las 25 del Frente C — otras 5 al azar entre los `ia_fit='fit'` de prod) en HITL completo: research → find_contacts → generate_draft → cola aprobación. **Monitor de falsos positivos classify_descr** (PM nota tras paso 4b smoke): RUTHERFORD ESPAÑOLA (piscinas) salió con `ia_fit='fit'` cuando su negocio no encaja con el ICP. Si en las 5 T3 de paso 6 aparece >1 falso positivo similar, eso indica que el clasificador IA de Sprint 3 necesita iterar (Haiku → Sonnet, prompt más estricto, o muestreo manual con Gonzalo) ANTES de paso 7. Si solo aparece 1 o ninguno, queda como ruido aceptable y seguimos.
