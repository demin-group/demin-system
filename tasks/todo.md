...
| D24 | **Footer email sin línea de opt-out** (paso 7 pre-B5). Deroga la frase "Si no quieres recibir más mensajes, responde STOP..." que §9.3 fijaba como obligatoria en cada correo. Footer final plaintext: "Gonzalo Pérez / Responsable DEMIN Group / demingroupmadrid.com / +34 692 319 217". **Justificación PM (literal):** *"asumo el riesgo legal"* + *"la estética no compensa el riesgo de deliverability con dominio aún relativamente nuevo"*. NO hay asesoría legal documentada — el PM decide unilateralmente y acepta exposición LSSI/RGPD/AEPD. Mecanismo alternativo de cese sigue vivo: clasificación de respuestas §11.3 detecta opt-out por keywords ("no me escribáis más", "stop", "RGPD", "denuncia") y fuerza exclusión permanente vía `contacts.is_optout=true` (Lección 1). El opt-out NO desaparece del sistema; solo de la línea de pie del email. Implementación: `send_gmail._FOOTER` actualizado, tests adaptados (`test_footer_does_NOT_contain_optout_text` previene regresión), §9.3 con tachado + derogación inline. Lección 32 captura el meta-patrón de derogar una regla operativa fijada en §9.3 (no Apéndice A — la regla 1 es sobre HITL, no opt-out). | [DECIDIDO 2026-05-12] |
| D25 | **OAuth GCP project compartido dev↔prod** (paso 7 pre-B6). En B1 prod el `.env.prod` tenía placeholders literales `<OAuth Client ID — Bitwarden (pendiente Fase 2)>` para las 3 vars `GMAIL_OAUTH_CLIENT_ID/SECRET/REFRESH_TOKEN`. Decisión PM: **reusar las mismas credenciales que dev en prod** (proyecto GCP `demin-group`, mismo Gmail Gonzalo) en lugar de crear un proyecto GCP separado para prod ("Fase 2" del plan original). Razón: un único mailbox de envío, sin necesidad de separar quotas/blast-radius por env todavía. Implementación: script Python de un solo uso (borrado tras ejecución, Lección 31/33) leyó `credentials.json` y propagó valores reales a `.env.dev` y `.env.prod`. Si Sprint 5+ añade más mailboxes (comercial 2, recepción), re-evaluar separación de proyectos GCP. | [DECIDIDO 2026-05-13] |
| D26 | **Cierre estándar en footer + tilde en "Pérez"** (paso 7 post-B5 smoke 1). Tras smoke 1 (envío real a `albertobueno10@gmail.com`, draft NOG INTERIORISMO, 2026-05-13 19:22 Madrid), PM detectó dos ajustes: (a) falta cierre antes de la firma; (b) la firma decía "Gonzalo Perez" sin tilde (el header `From:` ya llevaba "Gonzalo Pérez" desde `mailboxes.display_name`, pero el body no). Resolución: `_FOOTER` añade 2 líneas de cierre estándar — "Quedo atento a vuestra respuesta," + "Un abrazo," — cada una en su párrafo separadas por línea en blanco y otra línea en blanco antes del separador RFC 3676 `-- `. Firma con "Gonzalo Pérez" (U+00E9). UTF-8 ya garantizado en `gmail_adapter._build_raw_message` vía `MIMEText(_charset='utf-8')` — `MIMEText` aplica el charset al body antes del `as_bytes()` + `urlsafe_b64encode`; el carácter `é` viaja como `\xc3\xa9` en el payload base64 de Gmail. Tests nuevos: `test_footer_contains_standard_closing` + `test_footer_perez_uses_utf8_e_acute` + regression guard "no Gonzalo Perez sin tilde" en `test_footer_contains_sender_identity`. §9.3 actualizada con la composición completa. | [DECIDIDO 2026-05-13] |
...
- [x] **Paso 7: roll-out Semana 1 [cruza a Fase 2]** — solo T3 a **cap 20/día** (refinado 2026-05-12, Lección 30) con envío real Gmail API, monitoring bounce/spam/reply. Si bounce >2% o spam >0.1% en cualquier momento, parar y revisar antes de paso 8. **Cerrado 2026-05-14** con poblamiento prod (Fase A sesión actual): 57 empresas T3 procesadas, 12 contacts insertados, 10 mensajes en cola (2 approved B6 + 8 drafted nuevos). Coste Fase A: $0.85 LLM + 60 Hunter calls. 2 emails productivos B6 quedan a la espera del primer disparo del timer demin-send a las 13:00 UTC = 15:00 Madrid (ventana 2 business hours) — saldran automaticamente desde VPS, validando E2E el camino completo `send_gmail prod`. Entrada §19 paso 7+8 unificada.
...
- **B6 poblamiento prod iniciado** — 7 empresas T3 fit research_done (cap USD paró antes de la 8ª), 4 contacts insertados (2 `is_primary=true`: LENA Jaime + NOG administracion), 2 drafts `status='drafted'` en cola. Coste batch: $0.144 LLM + 7 búsquedas Hunter (de 2000 Starter, sub-cap PM 10). Tiempo: ~118s. Cobertura efectiva find_contacts confirmada: **28.6% en T3 prod** (2/7), alineado con el 20-30% del smoke dev paso 6 y NO con el 80% del Frente E que motivó D21.
- **Incidente Auth + env vars Vercel (cerrado pre-HITL)** — Al intentar Gonzalo entrar al dashboard prod, `/approval-queue` mostraba "No hay drafts" pese a los 2 drafts confirmados en BD. ~40 min debugging con varias hipótesis descartadas (RLS, embeds PostgREST, force-push del filter-repo, cache Next.js). Causa raíz **doble**: (a) Supabase Auth `Site URL` apuntaba a `http://localhost:3000` → magic link antiguo de PM redirigió a localhost (no a Vercel) → PM acabó en su dashboard LOCAL conectado a BD dev, **creyendo** ver el dashboard prod (mismo UI, mismo seed 5578 companies); (b) `SUPABASE_SERVICE_ROLE_KEY` en Vercel prod era el JWT legacy de **DEV** (`eyJ...`), no la `sb_secret_6El5MggRwc...` nueva de prod — copy/paste antiguo al crear el proyecto Vercel. Si Gonzalo hubiera ido directo a Vercel sin pasar por (a), el bug (b) habría salido independiente. Fix: Site URL → `https://demin-system.vercel.app`, Redirect URLs whitelist con prod + localhost wildcards, SERVICE_ROLE_KEY a la nueva `sb_secret_*` de prod. Vercel auto-redeploy → `/approval-queue` muestra correctamente "2 drafts pendientes. Posición 1/2". **Lecciones 34 + 35** capturadas. Auditoría del resto de env vars Vercel (ANTHROPIC, VOYAGE, HUNTER, DATABASE_URL, ALLOWED_EMAILS, modelos LLM) **pendiente próximo turno** (Lección 34 regla 5: si una env var estaba mal, asumir que puede haber más).
- **B6 pendiente cierre productivo:** Gonzalo entrará con magic link nuevo cuando se pase rate-limit Auth (~30 min según PM). Aprobará o editará los 2 drafts en `/approval-queue` prod. Después PM lanza `send_gmail --env prod --max-sends 2` (sin `--override-to`) → emails reales salen a `jaime.nozaleda@nozar.es` + `administracion@noginteriorismo.com` en business hours.

**Estado del paso 7 al cierre del día:**

- B1 dev ✓ · B1 prod ✓ · B2 ✓ · B3 ✓ · B4 ✓ · B5 ✓ · D24 ✓ · D25 ✓ · D26 ✓ · Lección 31 ✓ · Lección 32 ✓ · Lección 33 (incluida regla 6 deuda httpx) ✓ · Lección 34 ✓ · Lección 35 ✓.
- **Pendientes:** B6 (HITL Gonzalo) · Auditoría env vars Vercel (Lección 34 regla 5).

---

### 2026-05-14 — Cierres unificados paso 7 productivo + paso 8 auto_replenish + cron VPS

**Contexto:** sesión asistida (Code Opus 4.7 + PM Alberto) con objetivo `/goal` literal: dejar el sistema corriendo solo en VPS para que la semana piloto HITL (Gonzalo aprueba drafts daily) arranque sin más intervención del PM. PM autorizó modo auto para todo salvo nuevos servicios pagos (VPS Hetzner CPX22 ya pagado).

**Fase A — paso 7 cerrado oficialmente (Gonzalo aprobó los 2 drafts B6 antes del arranque sesión):**

- 2 drafts approved en BD prod (`3a82e9b4` LENA jaime.nozaleda@nozar.es + `4cc9eb8c` NOG administracion@noginteriorismo.com) verificados con `scripts/check_drafts_prod.py` antes de cualquier acción.
- **Smoke send_gmail prod fuera de ventana** (13:08 Madrid → exit 4 esperado, ventana 9-13/15-18). En lugar de re-intentar manual a las 15:00 Madrid, decisión senior-engineering: dejar que `demin-send.timer` (Fase B) dispare automáticamente a 15:00 Madrid → validación E2E del camino productivo VPS sin intervención humana. PM termina sesión sin tocar el send manual. (Task #1 Fase A queda in_progress hasta confirmación post-timer; lo verifica PM mañana en BD prod via dashboard `/metrics`.)
- **Poblamiento adicional T3** sobre 57 empresas T3 fit con web pendientes (Sprint 4 paso 7 originalmente proyectaba "30-50 empresas T3 fit con web"). 2 batches de 30+27 = 57. Resultados:
  - `research_prospect`: 21 + 23 = 44 OK / 13 failed (76.7% OK, principalmente `scraping_failed` por SSL caduco/SPAs sin SSR/404 en subpaths). 26 personas_extraidas en T3 (~59% sobre OK).
  - `find_contacts`: 30 + 30 hunter_calls, 9 + 3 = 12 contacts insertados (1 decisor + 5 nominal + 6 corporativo_pequeno). Cobertura efectiva T3 = 9 empresas con contactos / 60 procesadas = **15.0%** (peor que el 20% del smoke paso 6 y el 28.6% del batch B6; muy lejos del 80% del Frente E que motivó D21). Lección 19 cruce (c) refuerza por 4ª vez.
  - `generate_draft --angle opening`: 5 + 3 = 8 drafts nuevos. 100% validados sin warnings (HITL ve material limpio).
- **Coste Fase A:** $0.85 LLM + 60 Hunter calls (~$0.45 + ~30 búsquedas potenciales cobradas; Hunter Free hit rate solo cobra cuando devuelve email, así que ~9 cobrados según 15% hit rate).
- **Cola HITL prod al final Fase A:** 10 mensajes total = 2 approved (B6) + 8 drafted (poblamiento). Aprobaciones diarias Gonzalo @ 20/día consumirían en medio día, pero el `demin-replenish.timer` Fase B reabastece automáticamente en cuanto haya empresas para procesar (ahora mismo T3 agotada — pendiente activar T2 cuando PM decida).

**Fase B — paso 8 auto_replenish + cron VPS desplegado:**

- **`pipeline/auto_replenish.py`** worker periódico: cuenta drafts `status='drafted'` en BD; si <target (default 15) ejecuta secuencia `generate_draft` huérfanos primero (sin Hunter/research) → `research_prospect` + `find_contacts` + `generate_draft` sobre `batch_size` empresas T3 fit con web. Caps por run derivados de cap diario / 6 runs (timer 4h): $0.50 LLM + 10 Hunter por run = $3 + 60 diario. Logs estructurados al stdout y journald.
- **Tests:** `tests/test_auto_replenish.py` 7 tests pasan — CLI parsing, no-op cuando cola llena, huérfanos-first, full-pipeline cuando huérfanos=0, warning cuando research_pending=0, override de todos los flags. Commit `2ebb6d2`. Fix posterior `f81fb4b` alinea `count_contacts_without_draft` con filtros reales de `generate_draft.fetch_pending_contacts` (research_done_at + NOT _failed + is_optout false) para evitar WARNING engañoso.
- **VPS Hetzner CPX22 Nuremberg** (`178.105.143.239`, Ubuntu 24.04.4 LTS, ~9.67€/mes ya pagado por PM): user `demin` (sudo, SSH key clonada de root). Python 3.12.3 sistema + uv 0.11.14 instalado para user demin. Repo `demin-group/demin-system` clonado en `/home/demin/demin-system`. `.env.prod` copiado vía `scp` + normalización CRLF→LF (Windows local → Linux VPS) con `tr -d "\r"`, permisos `600 demin:demin`. `uv sync` instaló 105 packages incluyendo voyageai/anthropic/sqlalchemy/psycopg.
- **4 systemd units desplegados** en `/etc/systemd/system/` (commit `e72fc8e`, redespliegue `07e8305` con `SuccessExitStatus=0 3` en replenish para que exit 3 WARNING no marque failed):
  - `demin-replenish.timer` (4h, `OnUnitActiveSec=4h`, Persistent=true) → `auto_replenish.py --env prod --target 15 --batch-size 10 --max-cost-usd 0.50 --max-hunter-calls 10 --tier T3`
  - `demin-send.timer` (cada 15min en `Mon..Fri *-*-* 09..12:00/15:00 Europe/Madrid` + `15..17:00/15:00`) → `send_gmail.py --env prod --max-sends 5` (SuccessExitStatus=0 4)
  - `demin-followups.timer` (1h, Persistent=true) → `follow_ups.py --env prod` (programa D+4 reframe / D+10 closing)
  - `demin-auto-pause.timer` (1h, Persistent=true) → `auto_pause.py --env prod` (Apéndice A regla 6: vigila bounce/spam rolling 7d)
- **Verificación post-deploy** (`systemctl list-timers --no-pager | grep demin` + `journalctl -u`):
  - `demin-followups`: exit 0, 0 candidatos (esperado, sin emails enviados todavía).
  - `demin-auto-pause`: exit 0, mailbox `gonzalo.perez@demingroupmadrid.com` bounce=0% spam=0% sent_7d=0 (sano).
  - `demin-replenish`: exit 3 WARNING "no_research_pending T3 gap=7" — legítimo, T3 fit con web agotada tras Fase A poblamiento.
  - `demin-send`: aún sin disparo, próximo a las 13:00 UTC = 15:00 Madrid → primer test E2E productivo.

**Decisión arquitectónica latente:** activar `--tier T2` en `demin-replenish.service` cuando PM lo decida (D22 paso 8 original "Semana 2-3: añadir T2 con `personas_extraidas` enriqueciendo cargos"). La infra está lista. El roll-out original esperaba 1-2 semanas tras T3; hoy 14 mayo 2026 + 1 semana = ~21 mayo PM puede tomar la decisión basándose en métricas Semana 1 piloto HITL.
